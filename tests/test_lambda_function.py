...
def test_lambda_handler_creates_a_newly_named_file_in_bucket(test_s3_setup):
    event = {
    ...
    assert test_s3_setup.get_object(Bucket='my_ingestion_bucket', Key=new_name)
...
def test_lambda_handler_fully_obfuscates_multiple_pii_fields_in_column(test_s3_setup):
    event = {
        'file_to_obfuscate': 's3://my_ingestion_bucket/new_data/file1.csv',
        'pii_fields': ['name', 'email_address']
    }
